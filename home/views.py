...
class HomeView(BaseView):
    def get(self, request):
        ...
        self.view['hots'] = Product.objects.filter(
            labels='hot', status='active')
        self.view['sales'] = Product.objects.filter(
            labels='sale', status='active')
        self.view['news'] = Product.objects.filter(
            labels='new', status='active')
...
class SubCategoryView(BaseView):
    def get(self, request, slug):
        ...
        self.view['subcategory'] = SubCategory.objects.filter(
            category_id=cat_id)
        self.view['subcat_products'] = Product.objects.filter(
...
class SearchView(BaseView):
    def get(self, request):
        if request.method == 'GET':
            ...
            self.view['search_product'] = Product.objects.filter(
...
def add_to_cart(request, slug):
    if Cart.objects.filter(slug=slug, user=request.user.username, checkout=False).exists():
        quantity = Cart.objects.get(
            slug=slug, user=request.user.username, checkout=False).quantity
        quantity = quantity + 1
        Cart.objects.filter(slug=slug, user=request.user.username,
                            checkout=False).update(quantity=quantity)
...
def deletecart(request, slug):
    if Cart.objects.filter(slug=slug, user=request.user.username, checkout=False).exists():
        Cart.objects.filter(
            slug=slug, user=request.user.username, checkout=False).delete()
